...
namespace engine::physics {
    ...
    {
        auto grid_x = static_cast<int32_t>(std::floor(x * inv_cell_size));
        auto grid_y = static_cast<int32_t>(std::floor(y * inv_cell_size));
                                                                   ...
                                                                   float height) const
    {
        ...
        return getObjectGridKeys(x - margin, y - margin, width + (2 * margin),
                                 height + (2 * margin));
    }
    ...
    PhysicsEngine::SpatialGrid::getObjectGridKeys(float x, float y, float width, float height) const
    {
        ...
        auto min_x = static_cast<int32_t>(std::floor(x * inv_cell_size));
        auto max_x = static_cast<int32_t>(std::floor((x + width) * inv_cell_size));
        auto min_y = static_cast<int32_t>(std::floor(y * inv_cell_size));
        auto max_y = static_cast<int32_t>(std::floor((y + height) * inv_cell_size));
    ...
    void PhysicsEngine::update(float delta_time)
    {
        ...
        for (auto* pc : components_) {
            if ((pc == nullptr) || !pc->isEnabled()) { // 检查组件是否有效和启用
                continue;
            }
    ...
    void PhysicsEngine::checkTileTriggers()
    {
        ...
        for (auto* pc : components_) {
            if ((pc == nullptr) || !pc->isEnabled()) continue; // 检查组件是否有效和启用
            auto* obj = pc->getOwner();
            if (obj == nullptr) continue;
            auto* cc = obj->getComponent<engine::component::ColliderComponent>();
            if ((cc == nullptr) || !cc->isActive() || cc->isTrigger()) {
                ...
                for (int x = start_x; x < end_x; ++x) {
                    for (int y = start_y; y < end_y; ++y) {
                        ...
                        if (tile_type == engine::component::TileType::HAZARD) {
                            ...
                            if (!triggered_pairs.contains(trigger_pair)) {
    ...
    void PhysicsEngine::applyWorldBounds(engine::component::PhysicsComponent* pc)
    {
        ...
        // 验证组件有效性
        if ((cc == nullptr) || (tc == nullptr) || !cc->isActive()) {
            return;
        }
        ...
        if (new_position.x < bounds_left) {
            ...
            pc->velocity_.x = std::max(pc->velocity_.x, 0.0F);
        }
        ...
        // 检查右边界
            ...
            position_changed = true;
            pc->velocity_.x = std::min(pc->velocity_.x, 0.0F);
        }
        ...
        if (new_position.y < bounds_top) {
            ...
            position_changed = true;
            pc->velocity_.y = std::max(pc->velocity_.y, 0.0F);
        }
                                                    ...
                                                    TileCollisionContext &context)
    {
        ...
        if ((tc == nullptr) || (cc == nullptr) || cc->isTrigger()) return false;
    ...
    bool PhysicsEngine::isSlopeTile(engine::component::TileType tile_type)
    {
        switch (tile_type) {
    ...
    bool PhysicsEngine::isSolidTile(engine::component::TileType tile_type)
    {
        return tile_type == engine::component::TileType::SOLID;
    }

    bool PhysicsEngine::isGroundTile(engine::component::TileType tile_type)
    {
        return tile_type == engine::component::TileType::SOLID ||
               tile_type == engine::component::TileType::UNISOLID;
    }

    bool PhysicsEngine::isLadderTile(engine::component::TileType tile_type)
    {
                                              ...
                                              TileCollisionContext &context)
    {
        ...
        const float width = moving_right ? context.new_position.x + context.world_aabb_size.x -
    ...
    {
        // 验证移动对象
        if (move_obj == nullptr) return false;
        ...
        if ((move_tc == nullptr) || (move_pc == nullptr) || (move_cc == nullptr) ||
            !move_cc->isActive() || move_cc->isTrigger()) {
            return false;
        }

        // 验证固体对象
        if (solid_obj == nullptr) return false;

        solid_cc = solid_obj->getComponent<engine::component::ColliderComponent>();

        if ((solid_cc == nullptr) || !solid_cc->isActive() || solid_cc->isTrigger()) {
            return false;
        }
    ...
    bool PhysicsEngine::calculateSolidObjectCollisionData(SolidObjectCollisionContext &context)
    {
        // 计算中心点
