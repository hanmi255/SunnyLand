    ...
    bool Config::loadFromFile(const std::string &file_path)
    {
        ...
        if (!file.is_open()) {
            ...
            if (!saveToFile(file_path)) {
                spdlog::error("创建配置文件 '{}' 失败", file_path);
            }
            return false;
        }

        if (file.peek() == std::ifstream::traits_type::eof()) {
            spdlog::warn("配置文件 '{}' 为空，使用默认设置", file_path);
            return false;
        }
        ...
        try {
            ...
            spdlog::info("加载配置文件 '{}' 成功", file_path);
            return true;
        } catch (const std::exception &e) {
            spdlog::error("加载配置文件 '{}' 失败: {}，使用默认值", file_path, e.what());
            return false;
        }
    }
    ...
    void Config::fromJson(const nlohmann::json &json)
    {
        ...
        if (json.contains("input_mappings") && json["input_mappings"].is_object()) {
            ...
            try {
                ...
                auto input_mappings =
                    mappings_json.get<std::unordered_map<std::string, std::vector<std::string>>>();
                // 如果成功，则将 input_mappings 移动到 input_mappings_
                input_mappings_ = std::move(input_mappings);
                spdlog::trace("成功从配置加载输入映射。");
            } catch (const std::exception &e) {
                spdlog::warn(
                    "配置加载警告：解析 'input_mappings' 时发生异常。使用默认映射。错误：{}",
                    e.what());
            }
        } else {
            spdlog::trace(
                "配置跟踪：未找到 'input_mappings' 部分或不是对象。使用头文件中定义的默认映射。");
        }
    }
