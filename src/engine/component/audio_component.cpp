...
namespace engine::component {
    ...
    {
        if ((audio_player_ == nullptr) || (camera_ == nullptr)) {
    ...
    void AudioComponent::playSound(std::string_view sound_id, int channel, bool use_spatial)
    {
        ...
        const auto sound_path = sound_id_to_path_.contains(std::string(sound_id))
                                    ? sound_id_to_path_[(std::string(sound_id))]
                                    : sound_id;

        if (use_spatial && (transform_component_ != nullptr)) {
    ...
    {
        if (sound_id_to_path_.contains(std::string(sound_id))) {
