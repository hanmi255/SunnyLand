...
namespace engine::input {
    ...
    void InputManager::handleMouseButtonEvent(const SDL_MouseButtonEvent& button_event)
    {
        ...
        if (const auto it = input_to_actions_map_.find(button);
            it != input_to_actions_map_.end()) {
    ...
    bool InputManager::isActionHeldDown(const std::string& action_name) const
    {
        if (const auto it = action_states_.find(action_name); it != action_states_.end()) {
    ...
    void InputManager::addInputMapping(const std::string& action_name, const std::string& key_name)
    {
        // 尝试键盘映射
        ...
        // 尝试鼠标映射
        if (const Uint32 mouse_button = mouseButtonFromString(key_name);
            mouse_button != 0) {
        ...
        // 未知输入类型
        spdlog::warn("输入映射警告: 未知键或按钮名称 '{}' 用于动作 '{}'.",
                    key_name, action_name);
    }

    // --- 工具函数 ---

    SDL_Scancode InputManager::scancodeFromString(const std::string& key_name) noexcept
    {
    ...
    {
        const auto it = action_states_.find(action_name);
